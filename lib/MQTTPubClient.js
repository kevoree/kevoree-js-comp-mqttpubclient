   ...
   */
	...
	in_pub: function(msg) {
		...
		if (this.client) {
			...
			this.log.debug(this.toString(), '"' + this.getName() + '" publishing "' + msg + '" to "' + topic + '"');
		}
	},
    ...
    var parsed = false;
      ...
      if (msg.topic === undefined && msg.message === undefined) {
        this.log.error(this.toString(), '"' + this.getName() + '" incoming JSON message is malformed (msg.topic = '+msg.topic+', msg.message = '+msg.message+')');
      } else {
        parsed = true;
      }
    } catch (err) {
    ...
    if (parsed && this.client) {
      ...
      this.log.debug(this.toString(), '"' + this.getName() + '" publishing "' + msg.message + '" to "' + msg.topic + '"');
    }
	}
});
